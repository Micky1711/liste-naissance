...
namespace Own;
	...
	class Own 
	{
	    ...
	    {
		    $alphabet = "abcdefghijkmnpqrstuwxyzABCDEFGHJKLMNPQRSTUWXYZ123456789&é\"#{([-|_\@)]=}$*:/;?";
		...
		{	
			...
			if($long == 1)
				return $day." ".\Own\Own::month($month)." ".$year." ".$time;
			elseif($long == 2)
				return $time;
			elseif($long == 4)
				return substr($time,0,5);
			else
				return $day." ".\Own\Own::month($month)." ".$year;
		}
				...
				if($order == 0)
				 ...
				 {			 
				   	$list .= '<li id="list_'.$elem->id.'"><div><span class="disclose"><span></span></span>list_'.$elem->id.' :: '.$elem->title.'</div>'."\n";
	 					$list .= \Own\Own::recursive_array($elem->children, $type,1);
				   	$list .= '</li>'."\n";		
				}		
				$list .= '</ol>'."\n";			 
